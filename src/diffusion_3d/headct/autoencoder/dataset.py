...
class HeadCTDataModule(L.LightningDataModule):
    ...
    def create_dataloader(self, run_type):
        dataset = HeadCTDataset(self.config, run_type)
...
class HeadCTDataset(Dataset):
    ...
    def load_csv(self):
        ...
        if self.config.sources:
            self.dataset = self.dataset[self.dataset.Source.isin(self.config.sources)]
        if self.config.bodyparts:
            self.dataset = self.dataset[self.dataset.BodyPart.isin(self.config.bodyparts)]
