...
if task is not None:
    maisi_params = sum(p.numel() for p in model.autoencoder.maisi.parameters())
    ...
    any_other_params = total_params - (maisi_params + perceiver_params)
    task.add_tags(
        [
            f"MAISI: {maisi_params:,} params",
