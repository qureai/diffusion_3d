...
from diffusion_3d.chestct.diffusion.ldm.nn import LDM3D
...
class LDM3DLightning(MyLightningModule):
    def __init__(self, model_config: dict, training_config: Munch):
        ...
        self.vae = FrozenMaisiVAE()
        self.denoiser = LDM3D(model_config, training_config.checkpointing_level)
    ...
    def calculate_reconstruction_loss(self, pred, target):
        return {
    ...
    def calculate_basic_losses(self, pred, target):
        return self.calculate_reconstruction_loss(pred, target)

    @torch.no_grad()
    def calculate_train_metrics(self, pred, target):
        return {
            "cosine_similarity": self.cosine_similarity_metric(pred, target).mean(),
        }
    ...
    def process_training_step(self, batch, batch_idx=-1):
        x0, spacings, offsets = self.process_batch(batch)
        ...
        # Build guidance
        guidance_ratios = self.training_config.guidance_ratios
        noise_params_guidance = (torch.rand((batch_size,)) <= guidance_ratios["noise_params"]).to(x0)
        spacings_guidance = (torch.rand((batch_size,)) <= guidance_ratios["spacings"]).to(x0)
        offsets_guidance = (torch.rand((batch_size,)) <= guidance_ratios["offsets"]).to(x0)
    ...
    #             noise_pred,
    #             timesteps,
    #             eta=self.training_config.val_ddim_eta,
    #             prev_t_offset=self.training_config.val_ddim_skip_steps,
    #         )
    ...
    def forward(self, xt, t, spacings, offsets, guidance):
        o = self.denoiser(xt, t, spacings, offsets, guidance)
        return o
...
if __name__ == "__main__":
    ...
    autoencoder = LDM3DLightning(config.model, config.training).to(device)
